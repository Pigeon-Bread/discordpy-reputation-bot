...
from dbcontrol import showrep, addrep, adm_addrep, adm_remrep
...
async def show(ctx, user : discord.Member = None):
    if user == None:
        ...
        create_image(ctx.author.avatar_url, rep, ctx.author.display_name)
        await ctx.send(file=discord.File('result.png'))
    else:
        rep = showrep(user.id)
        create_image(user.avatar_url, rep, user.display_name)
        await ctx.send(file=discord.File('result.png'))
